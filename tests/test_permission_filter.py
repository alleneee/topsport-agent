...
@pytest.mark.asyncio
async def test_filter_passes_tools_with_subset_requirements():
    ...
    visible = await f.filter(tools, session)
    assert [t.name for t in visible] == ["read"]


@pytest.mark.asyncio
async def test_filter_empty_requirements_always_visible():
...
@pytest.mark.asyncio
async def test_filter_empty_grants_hides_all_guarded_tools():
    f = ToolVisibilityFilter()
    tools = [
...
@pytest.mark.asyncio
async def test_filter_requires_full_subset():
...
@pytest.mark.asyncio
async def test_filter_superset_grants_work():
    f = ToolVisibilityFilter()
